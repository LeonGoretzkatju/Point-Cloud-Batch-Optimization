%% BundleAdjustment.m
%%
close all;
clc;
clear;

%% Load Data
load Data/CarPark_Data/Scan_CarPark.mat;
load Data/CarPark_Data/Odom_CarPark.mat;
load Data/CarPark_Data/Odom_CarPark.mat;

%% Create Grid Map
Size_i = 700;
Size_j = 700;
Scale = 0.05;
Origin = [-13.5;-26.5]; %left/right hand coordinate frame?
[Map] = FuncCreateGridMap(Size_i,Size_j,Scale,Origin);
LAMBDA_O=1;

%% Initialise Grid Map
N_T = 2.0;
N_R = 0.5;
% N_T = 0.0;
% N_R = 0.0;
Pose = Pose_GT;
nP = size(Pose,1);
Pose(2:end,1:2) = Pose(2:end,1:2)+(rand(nP-1,2)-0.5)*N_T;
Pose(2:end,3) = Pose(2:end,3)+(rand(nP-1,1)-0.5)*N_R;

%%
% [Pose] = FuncGetInitialPose(Odom);

%%
[Map] = FuncInitialiseGridMap(Map,Pose,Scan);
Map = FuncSmoothN2(Map,10);
Map = FuncMapGrid(Map);

Diff1 = Pose-Pose_GT;

%% Show Initialisation
figure(1);
ab = exp(Map.Grid);
PMat = ab./(ab+1);
imshow(1-PMat);
fig_1 = 1- PMat;
imwrite(fig_1,'./initial_intel.jpg','jpg');
%% Least Squares
tic;

% [Map,Pose,Reason,Info] = FuncLeastSquaresLMSBA(Map,Pose,D,K);

% [Map,Pose,Reason,Info] = FuncLeastSquares(Map,Pose,Scan);
[Map,Pose,Reason,Info] = FuncLeastSquares22(Map,Pose,Scan,Odom,LAMBDA_O);

% [Map,Pose,Reason,Info] = FuncLeastSquaresPoseOnly(Map,Pose,D,K);

% [Map,Pose,Reason,Info] = FuncLeastSquaresFeatureOnly(Map,Pose,D,K);

%%
BATime = toc;
fprintf('BA Time Use %d\n\n', BATime);

%% Print Reason
fprintf('Reason is %d\n', Reason);
    
%% Show Result
figure(2);
ab = exp(Map.Grid);
PMat2 = ab./(ab+1);
imshow(1-PMat2);

%%
Diff2 = Pose-Pose_GT;
Diff = [Diff1;Diff2];
